...
export function renderGrapes(
	...
	TYPES(forms!).forEach(({id, ...data}) => 
		grapesInstance.DomComponents.addType(id, data)
	);
	...
	grapesInstance.on('component:update:traits', (component: any) => {
		if (component.changed.attributes?.fontSize && component.get('traits').where({name: 'fontSize'}).length) {
